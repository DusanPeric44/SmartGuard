...
{
    public static class SecurityActivityReportPdf
    {
        public static byte[] Render(SecurityActivityReportDocument document)
        {
            return Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    {
                        col.Item().Text("Security Activity Report").FontSize(18).SemiBold();
                    ...
                    page.Content().Column(col =>
                    {
                        ...
                        col.Item().Text("New Alarms").FontSize(14).SemiBold();
                        col.Item().Element(e => BuildAlertsTable(e, document));

                        col.Item().Text("New Persons").FontSize(14).SemiBold();
                        col.Item().Element(e => BuildPersonsTable(e, document));

                        col.Item().Text("New Face Detections").FontSize(14).SemiBold();
        ...
        private static void BuildAlertsTable(IContainer container, SecurityActivityReportDocument document)
        {
            if (document.Alerts.Count == 0)
            {
                container.Text("No new alarms in this period.");
                return;
            }
            ...
            container.Table(table =>
            {
                ...
                table.Header(header =>
                {
                    ...
                    header.Cell().Element(CellStyle).Text("Created (UTC)");
                });
                ...
                foreach (var a in document.Alerts)
                {
                    ...
                    table.Cell().Element(CellStyle).Text(a.DeviceName ?? string.Empty);
                    table.Cell().Element(CellStyle).Text(a.CreatedAtUtc.ToString("yyyy-MM-dd HH:mm:ss"));
                }
            });
        }

        private static void BuildPersonsTable(IContainer container, SecurityActivityReportDocument document)
        {
            if (document.KnownPersons.Count == 0)
            {
                container.Text("No new persons in this period.");
                return;
            }
            ...
            container.Table(table =>
            {
                ...
                foreach (var p in document.KnownPersons)
                {
                    table.Cell().Element(CellStyle).Text(p.Id.ToString());
                    table.Cell().Element(CellStyle).Text(p.FirstName ?? string.Empty);
                    table.Cell().Element(CellStyle).Text(p.LastName ?? string.Empty);
                    table.Cell().Element(CellStyle).Text(p.CreatedAtUtc.ToString("yyyy-MM-dd HH:mm:ss"));
                }
            });
        }

        private static void BuildFaceDetectionsTable(IContainer container, SecurityActivityReportDocument document)
        {
            if (document.FaceDetections.Count == 0)
            {
                container.Text("No new face detections in this period.");
                return;
            }
