...
{
    public static class WeeklyReportPdf
    {
        public static byte[] Render(WeeklyReportDocument document)
        {
            return Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    {
                        col.Item().Text("Weekly Report").FontSize(18).SemiBold();
                    ...
                    page.Content().Column(col =>
                    {
                        ...
                        col.Item().Text("New Devices").FontSize(14).SemiBold();
                        col.Item().Element(e => BuildDevicesTable(e, document));
        ...
        private static void BuildDevicesTable(IContainer container, WeeklyReportDocument document)
        {
            if (document.Devices.Count == 0)
            {
                container.Text("No new devices in this period.");
                return;
            }
            ...
            container.Table(table =>
            {
                ...
                foreach (var d in document.Devices)
                {
                    table.Cell().Element(CellStyle).Text(d.Id.ToString());
                    table.Cell().Element(CellStyle).Text(d.Name ?? string.Empty);
                    table.Cell().Element(CellStyle).Text(d.Location ?? string.Empty);
                    table.Cell().Element(CellStyle).Text(d.CreatedAtUtc.ToString("yyyy-MM-dd HH:mm:ss"));
                }
            });
        }

        private static void BuildAlertsTable(IContainer container, WeeklyReportDocument document)
        {
        ...
        private static void BuildFaceDetectionsTable(IContainer container, WeeklyReportDocument document)
        {
